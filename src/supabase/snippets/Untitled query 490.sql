...
begin
  ...
  if p_form_id is null then
    ...
    set date_agreement = v_date, city_agreement = v_city_agreement,
        type_person_agreement = v_type_person, person_doc_type = v_doc_type,
        person_doc_number = v_doc_number, status = v_status, payload = p_payload
    where f.id = p_form_id and f.user_id = auth.uid() and f.status = 'draft'
    returning f.id into v_id;
    if v_id is null then raise exception 'Form not found or not editable'; end if;
  end if;

  delete from public.sarlaft_relations where sarlaft_id = v_id;
  insert into public.sarlaft_relations (sarlaft_id, position, type_doc, doc_number, social_name, percentage_participation, activity_admin_resource, activity_reputation_grade_public)
  select v_id, (r.ordinality - 1)::int, nullif(r.value->>'typeDoc',''), nullif(r.value->>'docNumber',''), nullif(r.value->>'socialName',''), nullif(r.value->>'percentageParticipation','')::numeric, nullif(r.value->>'activityAdminResource',''), nullif(r.value->>'activityReputationGradePublic','')
  ...
  insert into public.sarlaft_account_entity_financials (sarlaft_id, position, account_type, account_number, account_name_entity)
  select v_id, (a.ordinality - 1)::int, nullif(a.value->>'accountType',''), nullif(a.value->>'accountNumber',''), nullif(a.value->>'accountNameEntity','')
  ...
  insert into public.sarlaft_foreign_products (sarlaft_id, position, type, entity, country, currency)
  select v_id, (f.ordinality - 1)::int, nullif(f.value->>'type',''), nullif(f.value->>'entity',''), nullif(f.value->>'country',''), nullif(f.value->>'currency','')
