<script lang="ts">
  ...
  const availableRoles = [
    ...
    { id: 'user', name: 'Usuario', description: 'Acceso limitado a funcionalidades básicas' }
  ];
...
<section class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8">
  <div class="max-w-6xl mx-auto">
    <!-- Encabezado -->
    <header class="mb-8">
      <h1 class="text-3xl md:text-4xl font-bold text-gray-800">Panel de Configuración</h1>
      <p class="text-gray-600 mt-2">Gestiona tu cuenta y configura las preferencias del sistema</p>
      
      <!-- Indicador de rol -->
      {#if isAdmin}
        <div class="inline-flex items-center mt-4 px-4 py-2 bg-purple-100 text-purple-800 rounded-full text-sm font-medium">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          <div class="flex flex-col items-center p-4">
            <div class="relative mb-6">
              ...
              />
              <div class="absolute bottom-2 right-2 bg-blue-500 text-white p-2 rounded-full cursor-pointer hover:bg-blue-600 transition">
            ...
            <div class="mt-6 pt-6 border-t border-gray-200 w-full">
              <h3 class="font-medium text-gray-700 mb-2">Información de la cuenta</h3>
              <div class="flex justify-between text-sm text-gray-600">
                <span>Miembro desde</span>
                <span class="font-medium">Enero 2023</span>
              </div>
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <Card
          ...
          content={editingProfile ? "Modifica tu información personal" : "Aquí puedes ver tu información personal"}
          class="h-auto"
        >
          <div class="p-4">
            {#if editingProfile}
        ...
        {#if isAdmin}
          <Card
            ...
            content={editingRole ? "Asigna o modifica el rol de usuario" : "Gestiona los permisos y roles de usuario"}
          ...
          >
            <div class="p-4">
              {#if editingRole}
                ...
                <div class="space-y-6">
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-3">Selecciona un rol para este usuario</label>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {#each availableRoles as role}
                        ...
                        >
                          <div class="flex items-center">
                            <div class={`w-4 h-4 rounded-full border flex items-center justify-center mr-3 ${selectedRole === role.id ? 'border-purple-500 bg-purple-500' : 'border-gray-300'}`}>
                              {#if selectedRole === role.id}
                                <svg class="w-2 h-2 text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                  <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                </svg>
                              {/if}
                            </div>
                  ...
                  <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-medium text-gray-700 mb-2">Permisos del rol seleccionado</h3>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {#if selectedRole === 'admin'}
                        <span class="inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded">Todos los permisos</span>
                        <span class="inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded">Gestionar usuarios</span>
                        <span class="inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded">Modificar configuración</span>
                        <span class="inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded">Acceso completo</span>
                      {:else if selectedRole === 'editor'}
                        <span class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Crear contenido</span>
                        <span class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Editar contenido</span>
                        <span class="inline-block bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">Gestionar usuarios</span>
                        <span class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Subir archivos</span>
                      {:else if selectedRole === 'viewer'}
                        <span class="inline-block bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded">Ver contenido</span>
                        <span class="inline-block bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">Crear contenido</span>
                        <span class="inline-block bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">Editar contenido</span>
                        <span class="inline-block bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">Gestionar usuarios</span>
                      {:else}
                        <span class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Ver contenido</span>
                        <span class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Comentar</span>
                        <span class="inline-block bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">Crear contenido</span>
                        <span class="inline-block bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded">Editar contenido</span>
                      {/if}
                    </div>
                ...
                <div class="space-y-4">
                  ...
                  <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                    <div class="flex">
                      ...
                      <div>
                        <p class="font-medium text-blue-800">Modo Administrador Activado</p>
                        <p class="text-sm text-blue-600 mt-1">Como administrador, puedes gestionar roles de usuario, permisos y configuraciones del sistema.</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="space-y-4">
              <div class="flex items-center justify-between py-3 border-b border-gray-100">
                <div>
                  ...
                  <p class="text-sm text-gray-500">Recibe notificaciones sobre actividad en tu cuenta</p>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" bind:checked={tempPreferences.notifications} class="sr-only peer">
              ...
              <div class="flex items-center justify-between py-3 border-b border-gray-100">
                <div>
                  ...
                  <p class="text-sm text-gray-500">Activa el tema oscuro para una mejor experiencia visual</p>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" bind:checked={tempPreferences.darkMode} class="sr-only peer">
                  <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                </label>
              </div>
              ...
              <div class="py-3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Idioma de la interfaz</label>
                    <select 
